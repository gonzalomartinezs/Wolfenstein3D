...
RocketLauncher::RocketLauncher(std::vector<Rocket>& rockets, TextureID texture,
                               float item_radius) :
                               Weapon(ROCKET_LAUNCHER, texture, item_radius),
                               rockets(rockets) {}
...
void RocketLauncher::fireTheGun(std::vector<Player*>& players, int shootingPlayerNumber, const Map& map) {
    if (players[shootingPlayerNumber]->hasBullets(BULLETS_PER_ROCKET)) {
        players[shootingPlayerNumber]->increaseBulletCounter(BULLETS_PER_ROCKET);
